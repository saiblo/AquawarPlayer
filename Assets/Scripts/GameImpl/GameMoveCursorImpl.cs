        ...
        public static void MoveCursor(this GameUI gameUI)
        {
            ...
            switch (coercedState)
            {
                case 2: // Current round is over
                {
                    ...
                    gameUI.resultText.text = $"{(gain > 0 ? 0 : 1)}号AI获胜";
                ...
                case 3: // Process Assertion
                {
                    ...
                    var subject = $"{((int) state["cur_turn"] == 0 ? 0 : 1)}号AI";
                    var target = $"{((int) state["cur_turn"] == 0 ? 1 : 0)}号AI";
                    if ((string) operation["Action"] == "Assert")
                    {
                        ...
                        gameUI.MakeAGuess(gameUI.GameState.AssertionPlayer == 0);
                        gameUI.ChangeStatus();
                        gameUI.AddLog(
